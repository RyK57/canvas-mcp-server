...
const startServer = (env) =>
  new Promise((resolve) => {
    const child = spawn("node", ["dist/index.js"], {
      ...
      env: {
        ...process.env,
        CANVAS_ACCESS_TOKEN: "1234~token",
...
const noToken = await new Promise((resolve) => {
  const child = spawn("node", ["dist/index.js"], {
    cwd: REPO_ROOT,
    env: { ...process.env, CANVAS_BASE_URL: "https://canvas.example.edu", CANVAS_ACCESS_TOKEN: "" },
    stdio: ["ignore", "pipe", "pipe"],
  });
  let stderr = "";
  child.stderr.on("data", (d) => (stderr += d.toString()));
...
check("missing token fails fast with setup guidance", noToken.code === 1 && noToken.stderr.includes("New Access Token"));

const noBaseUrl = await new Promise((resolve) => {
  const child = spawn("node", ["dist/index.js"], {
    cwd: REPO_ROOT,
    env: { ...process.env, CANVAS_ACCESS_TOKEN: "1234~token", CANVAS_BASE_URL: "" },
    stdio: ["ignore", "pipe", "pipe"],
  });
  let stderr = "";
  child.stderr.on("data", (d) => (stderr += d.toString()));
...
check("missing base URL fails fast", noBaseUrl.code === 1 && noBaseUrl.stderr.includes("no shared API host"));

const httpBaseUrl = await new Promise((resolve) => {
  const child = spawn("node", ["dist/index.js"], {
    cwd: REPO_ROOT,
    env: { ...process.env, CANVAS_ACCESS_TOKEN: "1234~token", CANVAS_BASE_URL: "http://canvas.example.edu" },
    stdio: ["ignore", "pipe", "pipe"],
  });
  let stderr = "";
  child.stderr.on("data", (d) => (stderr += d.toString()));
...
check("plain-http base URL refused", httpBaseUrl.code === 1 && httpBaseUrl.stderr.includes("https"), "token would cross the network in clear");
...
check("correct secret reaches the MCP server", correct.status === 200 && correctBody.result?.serverInfo?.name === "canvas-mcp-server");
